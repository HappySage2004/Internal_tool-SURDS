...
export function DocumentViewer({ doc, onClose, initialEditing = false }: DocumentViewerProps) {
  ...
  const [loading, setLoading] = useState(doc.content === undefined)
  const [editing, setEditing] = useState(initialEditing)
  const [saving, setSaving] = useState(false)

  // Lazily fetch the body if the list response didn't carry it.
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[10px] w-full max-w-2xl h-[80vh] flex flex-col shadow-xl">
          {/* Header */}
